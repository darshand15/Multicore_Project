...
bool merge_free_blks(book_keeping_node* bk_node)
{
...
int unmap(free_blk_list* free_blk)
{
    ...
    return munmap((void*)(bk_node), bk_node->size_mem_blk);
...
void* my_mem_alloc(size_t size)
{
    ...
    shared_heap_mutex.lock(); //locking the heap to control concurrent access to the shared heap
...
void my_mem_free(void* ptr)
{
    ...
    bool ret_merge = merge_free_blks(bk_node_free);
